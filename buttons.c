...
volatile int voltageScale = 4; //0-4 index for five voltage scales
volatile int timeScale = 11; // 0 -11 index for 12 time scales
...
void buttonFunc(void)
{
    ...
    // add buttons s1, s2, and joystick select
    gpio_buttons |= ((~GPIOPinRead(GPIO_PORTH_BASE, 0xff) & GPIO_PIN_1) << 1); // button s1
    gpio_buttons |= ((~GPIOPinRead(GPIO_PORTK_BASE, 0xff) & GPIO_PIN_6) >> 3); // button s2
    ...
    char letter;
    if (presses & 1)
    {    //EK-TM4C1294XL button 2
        letter = 'a';  // change spectrum mode
        Mailbox_post(mailbox0, &letter, BIOS_WAIT_FOREVER);
    }
    if (presses & 2)
    {  //EK-TM4C1294XL button 1
        letter = 'b';  // increase time
        Mailbox_post(mailbox0, &letter, BIOS_WAIT_FOREVER);
    }
    if (presses & 4)
    {    // BoosterPack button S1
        letter = 'c';  // increase  voltage
        Mailbox_post(mailbox0, &letter, BIOS_WAIT_FOREVER);
    }
    if (presses & 8)
    {   // BoosterPack button S2
        letter = 'd';  // decrease t voltage
        Mailbox_post(mailbox0, &letter, BIOS_WAIT_FOREVER);
    }
    if (presses & 16)
    {  // Joystick Select
        letter = 'e';  // change trigger slope
        Mailbox_post(mailbox0, &letter, BIOS_WAIT_FOREVER);
    }
    }
}

void uiFunc(UArg arg1, UArg arg2)
{
    IntMasterEnable();
    ...
    while (true)
    {
       ...
       if( Mailbox_pend(mailbox0, &buttons, BIOS_WAIT_FOREVER)){
        for (i = 0; i < 10; i++)
        {
            if (buttons[i] == ('a') && gButtons == 1) // usr_sw1 toggle spectrum
            {
                fftOn = !fftOn;
            }
            else if (buttons[i] == ('b') && gButtons == 2)
            {
                timeScale = (++timeScale) % 12; // increase time scale
            }
            else if (buttons[i] == ('c') && gButtons == 4) // booster s1
            {
                pwm_Period++;
            }
            else if (buttons[i] == ('d') && gButtons == 8) // booster s2
            {
                pwm_Period--;
            }
            else if (buttons[i] == ('e') && gButtons == 16) // joystick sel
            {
                PWMIntEnable(PWM0_BASE, PWM_INT_GEN_2); // play audio
            }
        }
       }
